...
| Pilot and baseband | Pilot tone frequency/deviation and audio deviation & pre-emphasis via `set_pilot` and `set_audio`. | Hard-coded to 19 kHz pilot, 6.75 kHz deviation, and 75 kHz audio deviation, with a UI toggle for 50 µs or 75 µs pre-emphasis. | Operators can match the curve to EU (50 µs) or US (75 µs) regions directly from the Audio tab.【F:si4713/__init__.py†L150-L162】【F:picast4713.py†L363-L379】【F:webapp/templates/index.html†L214-L236】【F:webapp/static/js/main.js†L776-L786】 |
...
| RDS identity & flags | PI, PTY, TP, TA, MS, and DI flags via `rds_set_pi`, `rds_set_pty`, `rds_set_tp`, `rds_set_ta`, `rds_set_ms_music`, and `rds_set_di`. | All parameters appear in the RDS PS tab with validation helpers. | The live metrics banner mirrors PI/PTY/flag state reported by the watchdog loop.【F:si4713/__init__.py†L198-L250】【F:webapp/transmitter.py†L262-L299】【F:webapp/templates/index.html†L236-L268】【F:webapp/transmitter.py†L830-L939】 |
...
2. **Composite, pilot, and audio deviation remain fixed.** Although the chip allows changing MPX composition, pilot tone, and audio deviation, PiCast still hardcodes those values in `apply_config`. Pre-emphasis is now operator-selectable between 50 µs and 75 µs via the dashboard.
